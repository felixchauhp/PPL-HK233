...
class StaticChecker (Visitor, Utils):
    ...
    def visitArrayLit(self, ast, param):
        ...
        if any(type(self.visit(x, param)) != type(first_type) for x in ast.explist[1:]):
        ...
        if isinstance(first_type, AutoType):
            for x in ast.explist:
                func = next(
                    (f for f in param[-1] if isinstance(f, Func) and f.name == x.name),
                    None
                )
                if func:
                    func.ret_type = self.typ.typ
                    self.func_call = func.name
                    self.save_stage = 0
                    param = self.visit(func.body, param)
                    self.save_stage = 0
                    self.func_call = None
